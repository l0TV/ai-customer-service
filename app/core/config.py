...
class Settings(BaseSettings):
    ...
    # HuggingFace 离线模式。
    # 默认 True：模型下载完成后即以离线方式加载，避免 sentence-transformers
    # 联网探测 adapter_config.json 等可选文件造成的超时重试
    # （实测首次加载由约 1s 被拖到 14s，网络不通时甚至会挂死整个请求）。
    # 首次下载模型时请临时设为 false，或先用 scripts/download_model.py 预下载。
    ...
    # 标定方式：运行 scripts/calibrate_threshold.py，用真实语料统计
    # 「相关问题」与「无关问题」的相似度分布，取两类之间的空档中点。
    #
    # 2026-09 实测（125 个片段 / 11 份政策文档，bge-small-zh-v1.5）：
    #   相关问题: 最低 0.5137  中位 0.6705  最高 0.7766
    #   无关问题: 最低 0.2398  中位 0.3735  最高 0.4067
    #   空档 = [0.4067, 0.5137]，取 0.45 使两端各留约 0.04 余量。
    #
    # 注意：语料变化会移动这个空档（曾用 0.40，补入 8 份通用性较强的政策文档后
    # 无关问题分数被抬高到 0.4089，导致漏放），因此增删文档后应重新标定。
